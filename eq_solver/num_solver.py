...
class DataBlock:
    ...
    def af_system(self, grid_count, norm_coeff=1):
        """
        Returns the Af system of the datablock.
        """
        self.af = system_af(self.ua_fit, self.ub_fit, self.ax, self.sigma, self.initial, grid_count, norm_coeff=norm_coeff)
        return self.af
...
def system_af(u_a, u_b, a_x, vola, initial, m, norm_coeff = 1.0):
    ...
    for i in range(m - 1):
        for j in range(1, m - 1):
            ...
            line_count += 1
    # norm the PDE part
    norm_value = np.linalg.norm(A, axis=1, ord=2) / norm_coeff
    norm_value[line_count:] = 1
    ...
    A = A / norm_value[:, None]
    # There's no need to normalize f, it is already normalized.
    return A, f
